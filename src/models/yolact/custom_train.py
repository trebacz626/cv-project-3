...
import wandb
wandb.init(project="my-test-project", entity="trebacz626")
# Oof
...
parser.add_argument('--save_folder', default='models/yolact/',
...
def train():
    ...
    try:
        compute_validation_map(0, 0, yolact_net, val_dataset)
        for epoch in range(num_epochs):
            ...
            for datum in data_loader:
                ...
                # datum = prepare_data(datum)
                images, targets, masks, num_crowds = prepare_data(datum)
                images, targets, masks, num_crowds = images[0], targets[0], masks[0], num_crowds[0]
                losses = net(images, targets, masks, num_crowds)
                
                losses = { k: v for k,v in losses.items() } # Mean here because Dataparallel
...
def compute_validation_map(epoch, iteration, yolact_net, dataset):
    with torch.no_grad():
        yolact_net.eval()
        ...
        wandb.log({"validation mAP box": val_info["box"]['all'], "validation mAP mask": val_info["mask"]['all']})
